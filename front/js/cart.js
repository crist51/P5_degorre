...
async function corp() {
  ...
  if (key != null) {
    ...
    function calculPrice(price) {
      var debutBoucle = 0;
      priceArray.length = 0;///reset la taille du tableau a chaque modif de l'input
      while (debutBoucle < key.length) {
        var priceTotalArticle = listePrix[debutBoucle].textContent * (key[debutBoucle].quantity);
